	...
	<div>
		<div class="bg-secondary bg-gradient w-100">
			<h2 class="text-center text-light p-2">
				<i class="fa fa-edit" aria-hidden="true"></i> Update Resume</h2>
		</div>
		<form class="w-50 m-4 shadow p-4 mx-auto" id="edit-resume-form" enctype="multipart/form-data" method="post">
			...
			<div class="mb-3">
				<div class="w-50 float-start p-2">
					<label for="fname" class="form-label">First Name</label> <input
						type="text" class="form-control" name="firstname" id="fname"
			...
			<div class="mb-3">
				<div class="w-50 float-start p-2">
					<label for="email" class="form-label"><i
						...
						value="<c:if test="${not empty mail}">${mail}</c:if>" />
				</div>
				<div class="w-50 float-end p-2">
			...
			<div class="mb-3">
				<div class="w-50 float-start p-2">
					<label for="year" class="form-label">Year</label> <select
						class="form-select" aria-label="Default select example" id="year" name="year">
				...
				<div class="w-50 float-end p-2">
					<label for="branch" class="form-label">Branch</label> <input
						...
						value="<c:if test="${not empty branch}">${branch}</c:if>" />
				</div>
			</div>
			<div class="p-2 w-50">
			...
			<div class="mb-3">
				<div class="p-2 float-start w-75">
					<label for="school" class="form-label">10th school name</label> <input
						...
						value="<c:if test="${not empty school}">${school}</c:if>" />
				</div>
				<div class="p-2 float-end w-25">
					<label for="tenth" class="form-label">10th GPA</label> <input
						type="number" class="form-control" name="tenth" id="tenth" min="0" step="any"
						value="<c:if test="${tenth>=0}">${tenth}</c:if>" />
				</div>
			</div>
			<div class="mb-3">
				<div class="p-2 float-start w-75">
					<label for="college" class="form-label">Intermmediate
						...
						id="college"
						value="<c:if test="${not empty college}">${college}</c:if>" />
				</div>
				<div class="p-2 float-end w-25">
					<label for="twelfth" class="form-label">12th percentage</label> <input
						type="number" class="form-control" name="twelfth" id="twelfth" min="0" step="any"
						value="<c:if test="${twelfth>=0}">${twelfth}</c:if>" />
				</div>
			</div>
			<div class="mb-3">
				<div class="p-2 float-start w-75">
					<label for="institute" class="form-label">Institute</label> <input
						...
						value="<c:if test="${not empty institute}">${institute}</c:if>" />
				</div>
				<div class="p-2 float-end w-25">
					<label for="btech" class="form-label">Btech CGPA</label> <input
						type="number" class="form-control" name="btech" id="btech" min="0" step="any"
						value="<c:if test="${btech>=0}">${btech}</c:if>" />
				</div>
			</div>
			<div class="mb-3">
			...
			<div class="mb-3 p-2">
				<input type="text" class="form-control mb-2" name="project1"
					...
					value="<c:if test="${not empty project1}">${project1}</c:if>" /> <input
					type="text" class="form-control mb-2" name="project2"
					placeholder="Description of Project 2" id="project2"
					value="<c:if test="${not empty project2}">${project2}</c:if>" /> <input
					type="text" class="form-control mb-2" name="project3"
					placeholder="Description of Project 3" id="project3"
					value="<c:if test="${not empty project3}">${project3}</c:if>" />
			</div>
			...
			<div class="mb-3 row p-2">
				...
				<div class="col-sm-4">
					<div>
						<input class="form-check-input" type="checkbox"
							id="cpp" name="cpp"
							<c:if test="${cpp=='on'}">checked</c:if>> <label
							class="form-check-label" for="cpp">cpp</label>
					</div>
					<div>
						<input class="form-check-input" type="checkbox"
							id="java" name="java"
							<c:if test="${java=='on'}">checked</c:if>> <label
							class="form-check-label" for="ai">Java</label>
					</div>
					<div>
						<input class="form-check-input" type="checkbox" id="py"
							name="python" <c:if test="${python=='on'}">checked</c:if> />
				...
				<div class="col-sm-4">
					<div>
						<input class="form-check-input" type="checkbox"
							id="react" name="react"
							<c:if test="${react=='on'}">checked</c:if> /> <label
							class="form-check-label" for="react">ReactJS</label>
					</div>
					<div>
						<input class="form-check-input" type="checkbox"
							id="ang" name="angular"
							<c:if test="${angular=='on'}">checked</c:if> />
						<label class="form-check-label" for="ang">Angular</label>
					</div>
					<div>
						<input class="form-check-input" type="checkbox"
							id="flutter" name="flutter"
							<c:if test="${flutter=='on'}">checked</c:if> />
			...
			<div class="mb-3 p-2">
				...
				<c:if test="${not empty interests}">
					<textarea class="form-control" rows="5" id="interests" name="interests" value="${interests}">${interests}</textarea>
				</c:if>
				
				<c:if test="${empty interests}">
					<textarea class="form-control" rows="5" id="interests" name="interests" value="${workexp}"></textarea>
				</c:if>
			...
			<div class="mb-3 p-2">
				...
				<div>
					<input class="form-check-input" type="checkbox" id="hk"
						name="hk" <c:if test="${hk=='on'}">checked</c:if> />
					<label class="form-check-label" for="hk">Hackerrank</label>
				</div>
				<div>
					<input class="form-check-input" type="checkbox" id="cc"
						name="cc" <c:if test="${cc=='on'}">checked</c:if> />
					<label class="form-check-label" for="cc">Code Chef</label>
				</div>
			</div>
			<div class="mb-3 p-2">
				<label for="linkedin" class="form-label"><i
					class="fa fa-linkedin" aria-hidden="true"></i> LinkedIn</label> <input
					type="url" class="form-control" name="linkedin" id="linkedin"
					value="<c:if test="${not empty linkedin}">${linkedin}</c:if>" />
			</div>
			<div class="mb-3 p-2">
				<label for="github" class="form-label"><i
					class="fa fa-github" aria-hidden="true"></i> Github</label> <input
					type="text" class="form-control" name="github" id="github"
					value="<c:if test="${not empty github}">${github}</c:if>" />
			</div>
			<div class="mb-3 mx-auto text-center">
				<button class="btn btn-danger text-center btn-block" formaction="CancelEditResumePage" id="edit-resume-form" type="submit">Cancel</button>
				<button class="btn btn-success text-center btn-block" formaction="SaveEditResumePage" id="edit-resume-form" type="submit">
